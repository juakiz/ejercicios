<?php
	...
	if ($accion == "eliminar" && isset($_REQUEST['usuario']) && isset($_REQUEST['tipo'])) {
		...
		$tipo = $_REQUEST['tipo'];
		if ($usuario == $profesor) {
			echo "<p class=\"titulo\" style=\"color:red; text-align:center;\">No puede eliminar el usuario que ha iniciado sesión</p>";
		} else {
	...
	function mostrarTablaUsuarios ($arrayUsuarios) {
		...
		for ($i = 0; $i < count($arrayUsuarios); $i++) {
			...
			echo "</td><td>";
				echo $tipo;
			echo "</td><td>";
				echo "<img src=\"imagenes/eliminar.png\" width=\"20\" height=\"20\" onClick=\"eliminarUsuario('".$usuario."', document.getElementById('tipoListar').value);\" onMouseEnter=\"this.style.backgroundColor='#EEEEEE';\" onMouseOut=\"this.style.backgroundColor='#CCCCCC';\">";
			echo "</td></tr>";
		}					
		echo "</table>";
	}	
